function F = in_fread_edf(sFile, SamplesBounds, ChannelsRange)
% IN_FREAD_EDF:  Read a block of recordings from a EDF/BDF file
%
% USAGE:  F = in_fread_edf(sFile, SamplesBounds, ChannelsRange)
%         F = in_fread_edf(sFile, SamplesBounds)               : Read all channels
%         F = in_fread_edf(sFile)                              : Read all channels, all the times
...
function F = edf_read_epoch(sFile, sfid, iEpoch, iTimes, ChannelsRange, isAnnotOnly, isBdfStatus)
    ...
    if (numel(F) < nReadTimes * nReadChannels)
        ...
        % Error message
        fprintf('EDF> ERROR: File is truncated (%d time samples were read instead of %d). Padding with zeros...', nTimeTrunc, nReadTimes);
        % Pad data with zeros 
    ...
    elseif ~isAnnotOnly
        ...
        F = double(F);
        % Apply gains
        F = bsxfun(@rdivide, F, [sFile.header.signal(iChannels).gain]');
        % Add offset
