function [sFile, ChannelMat, ImportOptions] = in_fopen_edf(DataFile, skipAnnot, ImportOptions)
        ...
        if strcmpi(sFile.format, 'EEG-EDF')
            ...
            for irec = 1:hdr.nrec
                for ichan = 1:length(iEvtChans)
                    % fprintf(["Reading annotations... [%d%%]", round((ichan + (irec-1)*length(iEvtChans))/length(iEvtChans)/hdr.nrec*100)]);
